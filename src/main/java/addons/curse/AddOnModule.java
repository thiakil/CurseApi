...
public class AddOnModule implements ADBBean {
	 ...
	 */
	public void setFingerprint(long param) {
		// setting primitive attribute tracker to true
		localFingerprintTracker = param != Long.MIN_VALUE;
	...
	public void serialize(final QName parentQName, XMLStreamWriter xmlWriter, boolean serializeType) throws XMLStreamException {
		...
		if (serializeType) {
			...
			if ((namespacePrefix != null) && (namespacePrefix.trim().length() > 0)) {
				Util.writeAttribute("xsi", "http://www.w3.org/2001/XMLSchema-instance", "type", namespacePrefix + ":AddOnModule", xmlWriter);
			} else {
				Util.writeAttribute("xsi", "http://www.w3.org/2001/XMLSchema-instance", "type", "AddOnModule", xmlWriter);
			}
		}

		if (localFingerprintTracker) {
			namespace = "Curse.AddOns";
			Util.writeStartElement(null, namespace, "Fingerprint", xmlWriter);

			if (localFingerprint == Long.MIN_VALUE) {
				throw new ADBException("Fingerprint cannot be null!!");
			} else {
				xmlWriter.writeCharacters(ConverterUtil.convertToString(localFingerprint));
			}
	...
	public static class Factory {
		 ...
		 */
		public static AddOnModule parse(XMLStreamReader reader) throws Exception {
			AddOnModule object = new AddOnModule();
			...
			try {
				...
				if (reader.getAttributeValue("http://www.w3.org/2001/XMLSchema-instance", "type") != null) {
					...
					if (fullTypeName != null) {
						...
						if (!"AddOnModule".equals(type)) {
							//find namespace for the prefix
							String nsUri = reader.getNamespaceContext().getNamespaceURI(nsPrefix);

							return (AddOnModule) ExtensionMapper.getTypeObject(nsUri, type, reader);
						}
					}
				}
				...
				if (reader.isStartElement() && new QName("Curse.AddOns", "Fingerprint").equals(reader.getName())) {
					...
					if ("true".equals(nillableValue) || "1".equals(nillableValue)) {
						throw new ADBException("The element: " + "Fingerprint" + "  cannot be null");
					}

					String content = reader.getElementText();

					object.setFingerprint(ConverterUtil.convertToLong(content));

					reader.next();
				} // End of if for expected property start element

				else {
					object.setFingerprint(Long.MIN_VALUE);
				}
