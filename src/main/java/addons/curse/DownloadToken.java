...
public class DownloadToken implements ADBBean {
	 ...
	 */
	public void setTimestamp(Calendar param) {
	...
	public void serialize(final QName parentQName, XMLStreamWriter xmlWriter, boolean serializeType) throws XMLStreamException {
		...
		if (serializeType) {
			...
			if ((namespacePrefix != null) && (namespacePrefix.trim().length() > 0)) {
				Util.writeAttribute("xsi", "http://www.w3.org/2001/XMLSchema-instance", "type", namespacePrefix + ":DownloadToken", xmlWriter);
			} else {
				Util.writeAttribute("xsi", "http://www.w3.org/2001/XMLSchema-instance", "type", "DownloadToken", xmlWriter);
			}
		}

		if (localIPAddressTracker) {
			namespace = "Curse.AddOns";
			Util.writeStartElement(null, namespace, "IPAddress", xmlWriter);
		...
		if (localTimestampTracker) {
			namespace = "Curse.AddOns";
			Util.writeStartElement(null, namespace, "Timestamp", xmlWriter);

			if (localTimestamp == null) {
				// write the nil attribute
				throw new ADBException("Timestamp cannot be null!!");
			} else {
				xmlWriter.writeCharacters(ConverterUtil.convertToString(localTimestamp));
			}
	...
	public static class Factory {
		 ...
		 */
		public static DownloadToken parse(XMLStreamReader reader) throws Exception {
			DownloadToken object = new DownloadToken();
			...
			try {
				...
				if (reader.getAttributeValue("http://www.w3.org/2001/XMLSchema-instance", "type") != null) {
					...
					if (fullTypeName != null) {
						...
						if (!"DownloadToken".equals(type)) {
							//find namespace for the prefix
							String nsUri = reader.getNamespaceContext().getNamespaceURI(nsPrefix);

							return (DownloadToken) ExtensionMapper.getTypeObject(nsUri, type, reader);
						}
					}
				}
				...
				if (reader.isStartElement() && new QName("Curse.AddOns", "IPAddress").equals(reader.getName())) {
					...
					if (!"true".equals(nillableValue) && !"1".equals(nillableValue)) {
						...
						object.setIPAddress(ConverterUtil.convertToString(content));
					} else {
