...
public class ArrayOfAddOnFile implements ADBBean {
		...
		if (serializeType) {
			...
			if ((namespacePrefix != null) &&
				...
				writeAttribute("xsi",
						...
						namespacePrefix + ":ArrayOfAddOnFile", xmlWriter);
			} else {
				writeAttribute("xsi",
						"http://www.w3.org/2001/XMLSchema-instance", "type",
						"ArrayOfAddOnFile", xmlWriter);
			}
		}

		if (localAddOnFileTracker) {
			if (localAddOnFile != null) {
				for (int i = 0; i < localAddOnFile.length; i++) {
					if (localAddOnFile[i] != null) {
						localAddOnFile[i].serialize(new QName(
								"Curse.AddOns", "AddOnFile"), xmlWriter);
					} else {
						writeStartElement(null, "Curse.AddOns", "AddOnFile",
								xmlWriter);
	...
	public static class Factory {
		 ...
		 */
		public static ArrayOfAddOnFile parse(
			...
			try {
				...
				if (reader.getAttributeValue(
					...
					if (fullTypeName != null) {
						...
						if (!"ArrayOfAddOnFile".equals(type)) {
							...
							return (ArrayOfAddOnFile) ExtensionMapper.getTypeObject(nsUri,
									type, reader);
						}
					}
				}
				...
				if (reader.isStartElement() &&
						new QName("Curse.AddOns",
								"AddOnFile").equals(reader.getName())) {
						...
						if (reader.isEndElement()) {
							...
							if (new QName("Curse.AddOns",
								...
								if ("true".equals(nillableValue) ||
									...
									list1.add(AddOnFile.Factory.parse(
											reader));
								}
							} else {
								loopDone1 = true;
					...
					object.setAddOnFile((AddOnFile[]) ConverterUtil.convertToArray(
							AddOnFile.class, list1));
