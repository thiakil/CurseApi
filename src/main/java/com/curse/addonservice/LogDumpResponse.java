...
public class LogDumpResponse implements ADBBean {
	...
	public void serialize(final QName parentQName, XMLStreamWriter xmlWriter, boolean serializeType) throws XMLStreamException {
		...
		if (serializeType) {
			...
			if ((namespacePrefix != null) && (namespacePrefix.trim().length() > 0)) {
				Util.writeAttribute("xsi", "http://www.w3.org/2001/XMLSchema-instance", "type", namespacePrefix + ":LogDumpResponse", xmlWriter);
			} else {
				Util.writeAttribute("xsi", "http://www.w3.org/2001/XMLSchema-instance", "type", "LogDumpResponse", xmlWriter);
			}
		}

		if (localLogDumpResultTracker) {
			if (localLogDumpResult == null) {
				Util.writeStartElement(null, "http://addonservice.curse.com/", "LogDumpResult", xmlWriter);
	...
	public static class Factory {
		 ...
		 */
		public static LogDumpResponse parse(XMLStreamReader reader) throws Exception {
			LogDumpResponse object = new LogDumpResponse();
			...
			try {
				...
				if (reader.getAttributeValue("http://www.w3.org/2001/XMLSchema-instance", "type") != null) {
					...
					if (fullTypeName != null) {
						...
						if (!"LogDumpResponse".equals(type)) {
							//find namespace for the prefix
							String nsUri = reader.getNamespaceContext().getNamespaceURI(nsPrefix);

							return (LogDumpResponse) ExtensionMapper.getTypeObject(nsUri, type, reader);
						}
					}
				}
				...
				if (reader.isStartElement() && new QName("http://addonservice.curse.com/", "LogDumpResult").equals(reader.getName())) {
					...
					if ("true".equals(nillableValue) || "1".equals(nillableValue)) {
						object.setLogDumpResult(null);
						reader.next();

						reader.next();
					} else {
						object.setLogDumpResult(ArrayOfstring.Factory.parse(reader));
